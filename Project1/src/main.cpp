...
int main()
{
	...
	//or std::sort(values.begin(),values.end(), [](int a, int b) {return a>b}
	std::sort(values.begin(), values.end(), [](int a, int b) {
		//to follow 1 to the end of the list
		//if a == 1, don't order it before b, move the back of b, 
		//if b ==1. move a to the front of the list
		if (a == 1) 
			return false;
		if (b == 1)
			return true;
...
//TUT70 casting in CPP
//static, dynamic, const, reinterpret cast.
//4 castings are meant to control the type transformation between classes(base and extened classes).
